...
type 'a expr =
  ...
  | While of 'a expr * 'a expr
    (** A while loop. *)
  | Declassify of 'a expr * 'a expr
    (** declassify command to release data *)
  | Output of 'a expr * var
    (** output command to print the data to a channel *)  
...
type input_event =
  ...
  | User_input_text_event of user_textbox * string * label option
...
(* Event as understood by the release module *)
type event = (input_event * value)