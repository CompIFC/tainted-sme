...
let script_hostpage = Add_handler(node_age, handler_hostpage)

let script_apage = Add_handler(node_age, handler_hostpage_secret)
...
let doc_list_apage = [
        Textbox(Some({elt_id_value="f-age"}), "0");
        Textbox(Some({elt_id_value="f-secret"}), "0");
] 
...
let html_file_inline_hostpage = Html_file( List.append doc_list_hostpage 
	[Inl_script(Some({elt_id_value="f"}), script_apage)]
)

let html_file_apage = Html_file(List.append doc_list_hostpage 
    [Rem_script(Some({elt_id_value="rem"}),
        Http_url(({domain_value=leak_domain},{req_uri_path={path_value=[]} ;
        req_uri_params=""})))        
        ] 
)

let script_file_remote_a = Script_file(script_apage)
...
(*   Network_response_event(Net_connection({domain_value=hostpage_domain},0),
  ...
  Network_response_event(Net_connection({domain_value=leak_domain},0),
        {resp_del_cookies = []; resp_set_cookies = [("key","value")]; resp_body=
                script_file_remote_a} ) ; 
 *)
...
let step ies bs oes =
  match bs with
        ...
        | (Running rbs) -> let (s2,oes2) = continue rbs (L, H) [] in Some(ies,s2,List.append oes oes2)
...
let rec run100Times n t =
    if n <= 0 then (Printf.printf "Average execution time: %fs\n" (Float.div t 1.0);)
