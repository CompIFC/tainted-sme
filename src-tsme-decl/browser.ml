...
(* Instead we should write 
 ...
 module Impl = struct 
  ...
  : inner expr =
    ...
    | Declassify (e1, e2) -> Declassify (f e1, f e2)
...
*)
  ...
  (** [node_new dn b] adds [dn] to the node store of [b] and returns its
      fresh key. *)
  let node_new (dn: node) (l:label) (b: b)
  : node_ref * b =
    let dr = ({node_ref_value=fresh_node_ref ();node_label=l}) in
  ...
  let rec build_node_tree (l:label) (doc: doc) 
  : node_ref * (node_ref * node) list =
    ...
    | Div(id, subdocs) ->
        let (drs, nhs) = List.split (List.map (build_node_tree l) subdocs) in
  ...
  let rec step_expr (ctx: context) (dCh:value list) (b: b) (e: inner expr) 
  : b * inner expr * output_event list * task list =
    let step = step_expr ctx dCh in
    begin match e with
    ...
    | X(Scoped_expr(ctx', e1)) ->
        let (b', e1', oes, ts) = step_expr ctx' dCh b e1 in
    ...
    | Declassify (Var(x), e2) ->
        (b, Set_var (x, String(List.hd dCh)), [], [])
    | Seq(X(R(_)), e2) ->
        (b, e2, [], [])
    ...
    | New_node(X(R(String_value("para")))) ->
        let (dr, b') = node_new (Para_node(None, "")) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])

    | New_node(X(R(String_value("link")))) ->
        let (dr, b') = node_new (Link_node(None, Blank_url, "")) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])

    | New_node(X(R(String_value("textbox")))) ->
        let (dr, b') = node_new (Textbox_node(None, "", [])) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])

    | New_node(X(R(String_value("button")))) ->
        let (dr, b') = node_new (Button_node(None, "", [])) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])

    | New_node(X(R(String_value("inl_script")))) ->
        let (dr, b') = node_new (Inl_script_node(None, Null, false)) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])

    | New_node(X(R(String_value("rem_script")))) ->
        let (dr, b') = node_new (Rem_script_node(None, Blank_url, false)) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])

    | New_node(X(R(String_value("div")))) ->
        let (dr, b') = node_new (Div_node(None, [])) ctx.context_label b in
        (b', X(R(Node_value(dr))), [], [])
  ...
  : state * output_event list =
    ...
    | Some((uri, dst)) ->
        ...
        | (Doc_dst(wr), Html_file(docs)) when win_valid wr b'1 ->
            ...
            let (dr, node_ext) = build_node_tree (M(d), M(d)) (Div(None, docs)) in
            let b'3 = {
              b'2 with
  ...
  let receive (ie: input_event) (w: waiting) (dCh:value list)
  : state * output_event list =
    begin match ie with
  ...
  let continue (r: running) (l:label) (dCh:value list)
  : state * output_event list =
    ...
    | t :: ts -> (* the task is current and not finished *)
        ...
        let (b', e', oes, ts') = step_expr ctx dCh b t.task_expr in
        let r' = {
          running_state = b';
